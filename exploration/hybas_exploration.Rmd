...
tar_load(google_historical) # gauge nowcast/historical
tar_load(google_historical_basin) # nowcast attached to each basin & classified by RP
...
**Forecast Data :** We currently have a data set containing **60** gauges that are being monitored by google. These are the final selection of gauges with highest prediction confidence. This data monitors river discharge for the current day and 1-7 day lead times.
...
google_historical_basin$hybas_af_lev04_v1c %>% 
...
```{r}
pct_gauges_gt_rp2_by_day<- google_historical_basin %>% 
  map(\(dft){
    dft %>% 
...
pct_gauges_flagged_day <- google_historical_basin %>% 
  map(\(dft){
    dft %>% 
      group_by(hybas_id = as.character(HYBAS_ID),
               yr=year(time),
...
tar_load(ghistorical_rp2_breach_pct_basin_gauges)
p_pct_breach_exceed <- ghistorical_rp2_breach_pct_basin_gauges %>% 
...
df_freq_breach_summary_basin <- ghistorical_rp2_breach_pct_basin_gauges %>% 
  map(\(dft){
    dft %>% 
      complete(
        hybas_id,
...
hybas_sel <- google_historical_basin$hybas_af_lev04_v1c %>% 
  clean_names() %>% 
  filter(
    hybas_id ==1040909890,
